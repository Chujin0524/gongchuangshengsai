...
static uint8_t (*ColorMapping)[4];  // 指向包含 4 个 uint8_t 元素的一维数组的指针
...
//    .ucBlobCenterErrCal = 10,    //转盘区校准误差允许（范围小，以便精准校准位置）
    .ucBlobCenterErrCal = 20,    //转盘区校准误差允许（范围小，以便精准校准位置）
...
//    .ucBlobStableThreshold = 3,  //转盘区稳定次数阈值，调节该参数以使小车不会随物块移动
    ...
    .usCircleCenter_X = 112,     //色环区中心X
    .usCircleCenter_Y = 112,     //色环区中心Y
...
void APP_MAIXCAM_Init( void )
{
    ...
    ColorMapping = APP_Handle_GetMapPoint();
}
...
{    
    ...
    dR = atan2f(240-s_tMaixCAM_Data.usCoord_Y, s_tMaixCAM_Data.usCoord_X-s_tMaixCAM_Data.usCircleCenter_X) * 180.0f / M_PI - 90;
    dX = calculate_hypotenuse(240-s_tMaixCAM_Data.usCoord_Y, s_tMaixCAM_Data.usCoord_X-s_tMaixCAM_Data.usCircleCenter_X) - (240-s_tMaixCAM_Data.usCircleCenter_Y);
...
//    if( abs( s_tMaixCAM_Data.dCoord_X ) <= s_tMaixCAM_Data.ucCircleCenterErrCal &&
//        abs( s_tMaixCAM_Data.dCoord_Y ) <= s_tMaixCAM_Data.ucCircleCenterErrCal )
    if( abs( s_tMaixCAM_Data.dCoord_R ) <= 2 &&
        abs( s_tMaixCAM_Data.dCoord_X ) <= 2 )
    {
        // 通知APP_Main并关闭摄像头
        xTaskNotifyGive(Task_MainHandle);
        xEventGroupClearBits( EventGroups_CarHandle, EventGroupsCarMaixCAM_EN_1 );     
    ...
    {        
        s_tMaixCAM_Data.dCoord_R = dR * 1;
        s_tMaixCAM_Data.dCoord_X = dX * 1;
        // 改变X R电机的绝对距离
        MOTOR_X_Pos[s_MaixCAM_ID.ucColor] += s_tMaixCAM_Data.dCoord_X;
        MOTOR_R_Pos[s_MaixCAM_ID.ucColor] += s_tMaixCAM_Data.dCoord_R;
        MOTOR_X_Pos[s_MaixCAM_ID.ucColor] = int32_constrain(MOTOR_X_Pos[s_MaixCAM_ID.ucColor], MOTOR_X_Pos_Min, MOTOR_X_Pos_Max);
        MOTOR_R_Pos[s_MaixCAM_ID.ucColor] = int32_constrain(MOTOR_R_Pos[s_MaixCAM_ID.ucColor], MOTOR_R_Pos_Min, MOTOR_R_Pos_Max);
        
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD,  10, 0, MOTOR_X_Pos[s_MaixCAM_ID.ucColor], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,      10, 0, MOTOR_R_Pos[s_MaixCAM_ID.ucColor], 1, 0); 
        vTaskDelay(1000);
    }
    
    App_Printf("dX=%.3f", dX);
    App_Printf("dR=%.3f", dR);
    App_Printf("dCoord_X=%d dCoord_R=%d", s_tMaixCAM_Data.dCoord_X, s_tMaixCAM_Data.dCoord_R);
    App_Printf("MOTOR_X_Pos[%d]=%d", s_MaixCAM_ID.ucColor, MOTOR_X_Pos[s_MaixCAM_ID.ucColor]);
    App_Printf("MOTOR_R_Pos[%d]=%d", s_MaixCAM_ID.ucColor, MOTOR_R_Pos[s_MaixCAM_ID.ucColor]);
}
...
static void SendToAPP_MotorWhale(void)
{
    ...
    ptCarXYR->WhalePosX = abs( s_tMaixCAM_Data.dCoord_X ) * 0.05;    

    ptCarXYR->WhaleDirY = CARDIRECTION_Y(s_tMaixCAM_Data.dCoord_Y);
    ptCarXYR->WhalePosY = abs( s_tMaixCAM_Data.dCoord_Y ) * 0.05;

    //向消息队列 Queue_MotorHandle 发送轮子参数
    xQueueOverwrite(Queue_MotorHandle, (void *)&ptCarXYR);    
}
...
{    
    ...
    {
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     10, 0, MOTOR_R_Pos[ColorMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 10, 0, MOTOR_X_Pos[ColorMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
        vTaskDelay(5000);
    }
