...
static uint16_t MOTOR_R_Vel = 50; 
...
static uint16_t MOTOR_X_Vel = 50; 
...
static uint16_t MOTOR_Y_Vel = 50; 
static uint32_t MOTOR_Y_Pos[8] = {0, 300, 1500, 1700, 9000, 6000, 8000, 0}; 

// 色环顺序映射表
// 色环顺序，与贴纸对应
uint8_t ColorMapping[2][4] = {0};  // [第i次加工区][红, 绿, 蓝] = [左中右]
...
{
    ColorMapping[_time][RED]   = RedDir;
    ColorMapping[_time][GREEN] = GreenDir;
    ColorMapping[_time][BLUE]  = BlueDir;
}
...
void APP_Handle_Place_All(uint8_t _time, uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    uint8_t ucSeq[3] = {ColorMapping[_time][_firstIndex], ColorMapping[_time][_secondIndex], ColorMapping[_time][_thirdIndex]};
    App_Printf("ColorSeq %d %d %d \r\n", _firstIndex, _firstIndex, _firstIndex);
    App_Printf("DirSeq %d %d %d \r\n", ucSeq[0], ucSeq[1], ucSeq[2]);
//      ColorMapping[0][] = B G R
...
void APP_Handle_Pick_All(uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    uint8_t ucSeq[3] = {ColorMapping[Fir][_firstIndex], ColorMapping[Fir][_secondIndex], ColorMapping[Fir][_thirdIndex]};
    App_Printf("ColorSeq %d %d %d \r\n", _firstIndex, _firstIndex, _firstIndex);
    App_Printf("DirSeq %d %d %d \r\n", ucSeq[0], ucSeq[1], ucSeq[2]);
