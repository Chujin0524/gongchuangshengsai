...
void chassis_control_vector( float* vx_set, float* vy_set, chassis_move_t* chassis_move )
{
    ...
    if ( chassis_move->dis_set >= 5000000)    //长距离行走时速度是快的
    {
        v_max = v1;
    }
    else                                      //短距离行走时速度是慢的
    ...
    chassis_move->dis_remaining -= chassis_move->dis;
    if( chassis_move->dis_set >= 5000000 && chassis_move->dis_remaining < 0.3*chassis_move->dis_set)    //长距离的最后5000000开始减速
    {
        vx_set_channel = 0.0f;
        vy_set_channel = 0.0f;
    }
    else if( chassis_move->dis_remaining <= 0)    //到达步数，减速停车
    {
        vx_set_channel = 0.0f;
        vy_set_channel = 0.0f;
    }
    ...
    if ( chassis_move->dis > chassis_move->dis_remaining)   //直接停车
    {
...
//        chassis_move_control->chassis_yaw_set = rad_format( angle_set );
        ...
        delat_angle = theta_format( delat_angle );
...
void APP_CHASSIS_MovePID( chassis_block_mode_e _block_mode, uint8_t _dir, int64_t _clk )
{
    ...
    if( chassis_move.dir == CarDirection_Rotate)
    {
        chassis_move.dis_set = chassis_move.dis_remaining = 0;
        chassis_move.chassis_yaw_set = _clk / 10000;
    }

    if( _block_mode == CHASSIS_CALL_BLOCKING )
    {
...
void APP_CHASSIS_Status( STATUS_E _ucStatus )
{
    ...
    case STATUS_Start_2_Qr:
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Left, 500 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 4200 );
        break;

    case STATUS_Qr_2_RawArea:
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 7000 );
        break;

    case STATUS_RawArea_2_ProFirArea:
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180 );    //夹爪朝向180度
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 2500 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 90 );     //夹爪朝向90度
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 28000 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180 );    //夹爪朝向180度
        break;

    case STATUS_ProFirArea_2_ProSecArea:
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0 );      //夹爪朝向0度
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 6500 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 270 );    //夹爪朝向270度
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 8000 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 90 );     //夹爪朝向90度
        break;
    ...
    case STATUS_ProSecArea_2_RawArea:
        ...
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 7000 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180 );    //夹爪朝向180度
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Forward, 1500 );
        APP_CHASSIS_MovePID( CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0 );      //夹爪朝向0度
        break;
