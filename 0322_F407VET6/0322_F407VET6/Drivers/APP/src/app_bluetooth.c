...
//        App_Printf( "g_fGyro_z : %.3f\r\n", g_fGyro_z );
    ...
    {
        APP_MOTOR_ZDT_Move_P( s_MotorZDT_ID, s_MotorZDT_DIR, 400, 0, s_MotorZDT_POS, 1, 0 );
    ...
    else if( strncmp(( char* )pucFrameData, "Handle_Test=", 12 ) == 0 )
    {
        ...
        {
            case 1:
                APP_Handle_Place_All( 1, 1, 2, 3 );    //第一次全部放置测试
                break;
            case 2:
                APP_Handle_Place_All( 2, 1, 2, 3 );    //第二次全部放置测试
                break;
            case 3:
