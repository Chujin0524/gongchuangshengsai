...
static uint16_t MOTOR_R_Vel = 100;
static uint32_t MOTOR_R_Pos[4] = {0, 6000, 3750, 1514};
...
static uint16_t MOTOR_X_Vel = 100;
static uint32_t MOTOR_X_Pos[4] = {2900, 6500, 3800, 6555};

//Y位置
static uint8_t MOTOR_Y_Acc = 250;
static uint16_t MOTOR_Y_Vel = 700;
static uint32_t MOTOR_Y_Pos[8] = {
    0,        //0 : 最高点
    0,        //1 : 最高点-300
    0,        //2 : 夹着物块能越过转盘的位置
    1600,     //3 : 夹着物块放到转盘上的位置
...
void APP_Handle_Place_All(uint8_t _time, uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    uint8_t ucSeq[3] = {DirMapping[_time-1][_firstIndex], DirMapping[_time-1][_secondIndex], DirMapping[_time-1][_thirdIndex]};
...
//      DirMapping[0][] = B G R
    ...
    {    
        ...
        if (i == 2)
        {
            ...
            {
                //Y5：物块在色环上，夹爪能越过的位置
                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel, MOTOR_Y_Acc, MOTOR_Y_Pos[0], 1, 0); APP_MOTOR_ZDT_WaitAck();
            }
            else if (_time == 2)
            {
                //Y1：最高点-300
