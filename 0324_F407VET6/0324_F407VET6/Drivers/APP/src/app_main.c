...
//            s_ucStatus = STATUS_Idle;
    ...
    case STATUS_ProFirAreaCal:
        ...
        //后处理2
//        APP_MAIXCAM_Cal( CIRCLE, ucaGrabSequence[0 + 3 * ucCycle],  0, MoveXR );        //识别蓝色圆环进行校准，使X、R电机移动
//        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     100, 250, 1514, 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 100, 250, 6555, 1, 0);
        vTaskDelay(500);
        APP_Handle_Place_One(ucaGrabSequence[0 + 3 * ucCycle]);
        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
    
//        APP_MAIXCAM_Cal( CIRCLE, ucaGrabSequence[1 + 3 * ucCycle],  0, MoveXR );        //识别蓝色圆环进行校准，使X、R电机移动
//        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
//        APP_Handle_Place_One(ucaGrabSequence[1 + 3 * ucCycle]);
//        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
//    
//        APP_MAIXCAM_Cal( CIRCLE, ucaGrabSequence[2 + 3 * ucCycle],  0, MoveXR );        //识别蓝色圆环进行校准，使X、R电机移动
//        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
//        APP_Handle_Place_One(ucaGrabSequence[2 + 3 * ucCycle]);
//        xTaskNotifyWait( 0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY );
