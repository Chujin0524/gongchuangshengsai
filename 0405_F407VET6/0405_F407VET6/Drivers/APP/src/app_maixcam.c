...
MAIXCAM_DATA_T s_tMaixCAM_Data =
{
    ...
    .usCircleCenter_Y = 120,     //中间色环区中心点Y

    .usRedCircleCenter_X = 168,     //红色环区中心点X
    .usRedCircleCenter_Y = 120,     //红色环区中心点Y
    .usGreenCircleCenter_X = 169,     //绿色环区中心点X
    .usGreenCircleCenter_Y = 119,     //绿色环区中心点Y
    .usBlueCircleCenter_X = 164,     //蓝色环区中心点X
...
static uint8_t APP_MAIXCAM_Parse_BLOB_ArriveAndWaitCal(void)
{
    ...
    s_tMaixCAM_Data.dCoord_X = s_tMaixCAM_Data.usPresentX - s_tMaixCAM_Data.usGreenCircleCenter_X;
    s_tMaixCAM_Data.dCoord_Y = s_tMaixCAM_Data.usPresentY - s_tMaixCAM_Data.usGreenCircleCenter_Y;

    g_dX = s_tMaixCAM_Data.dCoord_X;
    g_dY = s_tMaixCAM_Data.dCoord_Y;

    // 判断坐标是否连续n次不动
    if (abs(s_tMaixCAM_Data.usPresentX - s_tMaixCAM_Data.usPreviousX) < 5 && abs(s_tMaixCAM_Data.usPresentY - s_tMaixCAM_Data.usPreviousY) < 5)
    {
        // 坐标连续n次不动
    ...
    {
        if (abs(s_tMaixCAM_Data.usBlobCoord_X_Fir - s_tMaixCAM_Data.usPresentX) < 20 & abs(s_tMaixCAM_Data.usBlobCoord_Y_Fir - s_tMaixCAM_Data.usPresentY) < 20)
        {
            return 1;
        }
    }
...
static uint8_t APP_MAIXCAM_Parse_BLOB_Cal(void)
{
    if ( !s_tMaixCAM_Data.usCoord_X )   //如果收到的X Y数据有一个为0，则说明相机没有识别到，让摄像头重新发送
    {
        s_tMaixCAM_Data.ucSendFlag = 0;
        return 0;
    }
...
static uint8_t APP_MAIXCAM_Parse_CIRCLE_CalXR(void)
{
    ...
    else
    {
        ...
        s_tMaixCAM_Data.dCoord_R = dR * 20;
        s_tMaixCAM_Data.dCoord_X = dX * 20;
        // 改变X R电机的绝对距离
