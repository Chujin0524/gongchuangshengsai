...
volatile int Weak_mode = 0;          // 弱模式标志
...
int32_t w;                           // 差速值
float* gyro_z = 0;
volatile float angle_r = 0;                                             //电机角度值
volatile float angle_dia = 0;                                           //电机角度值
...
/* 定义位置环模式相关变量 */
volatile int position_mode_active = 0; // 位置环模式是否激活
volatile float target_angle[4] = {0};       // 目标角度
volatile float current_angle[4] ={0};      // 当前角度
volatile int position_mode_start_time = 0; // 位置环模式开始时间
volatile float angle_error[4]={0};
int flag=0;
/* 定义位置环PID控制器 */
...
//    HAL_CAN_ActivateNotification(&hcan1, CAN_IT_RX_FIFO0_MSG_PENDING );
    ...
    gyro_z = APP_HWT101_GetAnglePoint();
}
...
void M2006_CAL()
{
    distance_r = ( moto_chassis[0].total_angle - moto_chassis[1].total_angle - moto_chassis[2].total_angle + moto_chassis[3].total_angle ) / 4;
    distance_big_dia = (-moto_chassis[0].total_angle - moto_chassis[1].total_angle + moto_chassis[2].total_angle + moto_chassis[3].total_angle ) / 4;
    angle_r = ( moto_chassis[0].total_angle - moto_chassis[1].total_angle - moto_chassis[2].total_angle + moto_chassis[3].total_angle ) / 4;
    angle_dia = (-moto_chassis[0].total_angle - moto_chassis[1].total_angle + moto_chassis[2].total_angle + moto_chassis[3].total_angle ) / 4;
}
...
{
    float angle0 = angle_dia;
    if( dis >= 0 )
    {
        while( angle_dia < angle0 + dis )
        {
...
/* 初始化位置环PID控制器 */
void init_position_pid() 
{
    float pid_params[3] = {1.0, 0.1, 0.0}; // 根据实际情况调整PID参数
		int i=0;
		for(i=0;i<4;i++)
		{
			PID_init(&position_pid[i], PID_POSITION, pid_params, 1000, 0.1);
		}
}

/* 更新电机当前角度 */
void update_current_angle(float angle) 
{
	int i=0;
	for(i=0;i<4;i++)
	{
		    current_angle[i] = moto_chassis[i].angle;
	}
...
{
    //position_mode_start_time = HAL_GetTick() + delay_time; // 设置位置环模式开始时间
	int i=0;
    	for(i=0;i<4;i++)
	{
		    	target_angle[i] = moto_chassis[i].angle; // 设置目标角度
	}
...
{    
        // 计算角度误差
			...
			for(i=0;i<4;i++)
			{
				angle_error[i] = target_angle[i] - moto_chassis[i].angle;
				if(my_abs(angle_error[i]) <=500)
				{
					motor_pid[i].set=0;
				}
				else
					{
				position_pid[i].Ki = 0.2 + 0.6 * (( my_abs(angle_error[i]) -4096 ) / 4096 );
        // 执行PID计算
        PID_calc(&position_pid[i], angle_error[i], 0);
				motor_pid[i].set = position_pid[i].out;
				}
