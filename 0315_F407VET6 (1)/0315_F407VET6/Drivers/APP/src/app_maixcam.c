...
//    if( abs( s_tMaixCAM_Data.dCoord_X ) <= s_tMaixCAM_Data.ucCircleCenterErrCal &&
//        abs( s_tMaixCAM_Data.dCoord_Y ) <= s_tMaixCAM_Data.ucCircleCenterErrCal )
    if( abs( s_tMaixCAM_Data.dCoord_R ) <= 5 &&
        abs( s_tMaixCAM_Data.dCoord_X ) <= 5 )
    {
        // 通知APP_Main并关闭摄像头
    ...
    {        
        ...
        MOTOR_X_Pos[s_MaixCAM_ID.ucColor] = int32_constrain(MOTOR_X_Pos[s_MaixCAM_ID.ucColor], MOTOR_X_Pos_Min, MOTOR_X_Pos_Max);    //限幅
        MOTOR_R_Pos[s_MaixCAM_ID.ucColor] = int32_constrain(MOTOR_R_Pos[s_MaixCAM_ID.ucColor], MOTOR_R_Pos_Min, MOTOR_R_Pos_Max);    //限幅

        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD,  30, 0, MOTOR_X_Pos[s_MaixCAM_ID.ucColor], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,      30, 0, MOTOR_R_Pos[s_MaixCAM_ID.ucColor], 1, 0); 
        vTaskDelay(500);
    }
    
    App_Printf("dX=%.5f               dR=%.5f\r\n", dX, dR);
    App_Printf("dCoord_X=%5d          dCoord_R=%5d\r\n", s_tMaixCAM_Data.dCoord_X, s_tMaixCAM_Data.dCoord_R);
    App_Printf("MOTOR_X_Pos[%d]=%5d   MOTOR_R_Pos[%d]=%5d\r\n", s_MaixCAM_ID.ucColor, MOTOR_X_Pos[s_MaixCAM_ID.ucColor], s_MaixCAM_ID.ucColor, MOTOR_R_Pos[s_MaixCAM_ID.ucColor]);
}
...
{    
    ...
    {
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 0, MOTOR_R_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 0, MOTOR_X_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
        vTaskDelay(2000);
    }
