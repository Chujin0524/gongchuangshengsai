...
static void APP_BLUETOOTH_Parse(uint8_t* pucFrameData, uint8_t ucFrameLength)
{
    ...
    {
        APP_MOTOR_ZDT_Move_P(s_MotorZDT_ID, s_MotorZDT_DIR, 100, 50, s_MotorZDT_POS, 1, 0);
    ...
    else if (strncmp((char *)pucFrameData, "Handle_Test=", 12) == 0)
    {
        ...
        switch (test)
        {
            ...
            case 7:                  //放一个测试
                vTaskDelay(5000);
                APP_MOTOR_ZDT_Move_P(7, 0, 800, 0, 10500, 1, 0);
                APP_MOTOR_ZDT_WaitAck();  //降到最底
                APP_SERVO_Jaw('Z');        //夹紧
                APP_MOTOR_ZDT_Move_P(7, 0, 800, 0, 300, 1, 0);
                APP_MOTOR_ZDT_WaitAck();    //升到最高
                APP_SERVO_Wrist('R');
                vTaskDelay(400);     //手腕朝右
                APP_MOTOR_ZDT_Move_P(7, 0, 800, 0, 3000, 1, 0);
                APP_MOTOR_ZDT_WaitAck();   //降到转盘上分
                APP_SERVO_Jaw('S');       //松开
                APP_MOTOR_ZDT_Move_P(7, 0, 800, 0, 300, 1, 0);
                APP_MOTOR_ZDT_WaitAck();    //升到最高
                APP_SERVO_Wrist('F');
