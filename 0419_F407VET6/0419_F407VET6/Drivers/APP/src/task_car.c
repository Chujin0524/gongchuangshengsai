...
void AppTask_ChassisVelControl(void* argument);
...
//    xEventGroupSetBits(EventGroups_CarHandle, EventGroupsCarShow_7);    //用于决定显示任务是否工作
    ...
    for (;;)
    {
        ...
        if ((uxBits & (EventGroupsCarShow_7)) == (EventGroupsCarShow_7))
        {
            ...
            PRINT(speed, "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f"
...
{
    UNUSED(argument);
    const TickType_t xFrequency = 50; // 检测周期(ms)
    ...
    for (;;)
    {
        ...
        /* RESET键检测：高->低跳变触发 */
        static uint8_t resetKeyPressedFlag = 0;
        static GPIO_PinState resetKeyPrevState = GPIO_PIN_SET;
        GPIO_PinState resetKeyCurrentState = HAL_GPIO_ReadPin(RESET_KEY_GPIO_Port, RESET_KEY_Pin);
        
        // 检测下降沿
        if (resetKeyPrevState == GPIO_PIN_RESET && resetKeyCurrentState == GPIO_PIN_SET) 
        {
            resetKeyPressedFlag = 1; // 标记RESET键触发
        }
        else if (resetKeyPressedFlag == 1 && resetKeyPrevState == GPIO_PIN_SET && resetKeyCurrentState == GPIO_PIN_RESET) 
        {
            key = 1; // 标记RESET键触发
        }
        resetKeyPrevState = resetKeyCurrentState;
        
        
        
        
        /* START键检测：低电平保持到释放 */
        static uint8_t startKeyPressedFlag = 0;
        GPIO_PinState startKeyState = HAL_GPIO_ReadPin(START_KEY_GPIO_Port, START_KEY_Pin);
        
        // 按下阶段检测
        if (startKeyPressedFlag == 0 && startKeyState == GPIO_PIN_RESET) 
        {
            startKeyPressedFlag = 1; // 标记按下状态
        }
        // 释放阶段检测
        else if (startKeyPressedFlag == 1 && startKeyState == GPIO_PIN_SET) 
        {
            key = 2;        // 标记START键触发
            startKeyPressedFlag = 0; // 清除按下标记
        }
        ...
        if (key != 0)
        {
            ...
            if (key == 1) 
            {
                ...
                if (HAL_GPIO_ReadPin(RESET_KEY_GPIO_Port, RESET_KEY_Pin) == GPIO_PIN_RESET)
                {
                    ...
                    Emm_V5_Stop_Now(5, 0); HAL_Delay(5);
                    Emm_V5_Stop_Now(6, 0); HAL_Delay(5);
                    Emm_V5_Stop_Now(7, 0); HAL_Delay(5);
                    Emm_V5_Stop_Now(8, 0); HAL_Delay(5);
                    NVIC_SystemReset();
                }
            }
            // START键二次验证
