...
MAIXCAM_DATA_T s_tMaixCAM_Data =
{
    ...
    .usBlobCenter_X = 192,       //转盘区中心X
    .usBlobCenter_Y = 70,       //转盘区中心Y
    ...
    .usCircleCenter_X = 169,     //中间色环区中心点X
    .usCircleCenter_Y = 132,     //中间色环区中心点Y

    .usRedCircleCenter_X = 171,     //红色环区中心点X
    .usRedCircleCenter_Y = 132,     //红色环区中心点Y
    .usGreenCircleCenter_X = 169,   //绿色环区中心点X
    .usGreenCircleCenter_Y = 132,   //绿色环区中心点Y
    .usBlueCircleCenter_X = 170,    //蓝色环区中心点X
    .usBlueCircleCenter_Y = 130,    //蓝色环区中心点Y
    .ucCircleCenterErrCal = 1,   //色环区校准误差允许
    .ucCircleStableThreshold = 2, //色环区校准对准次数，调节该参数以使小车精确对准中心
    ...
    switch (s_tMaixCAM_Data.ucStatus)
    {
        ...
        case MAIXCAM_STATUS_BLOB_Cal:
            if (APP_MAIXCAM_Parse_BLOB_Cal() == 1)
            {
                ...
                tick_flag = (tick < 2000) ? 1 : 2;     //判断车是否在2500ms内校准完成
...
//        APP_MAIXCAM_SendAndRead(s_MaixCAM_ID.ucElement, s_MaixCAM_ID.ucColor);
    ...
    App_Printf("X %d  Y %d\r\n", s_tMaixCAM_Data.usCoord_X, s_tMaixCAM_Data.usCoord_Y);
    ...
    App_Printf("dR %.3f  dX %.3f\r\n", dR, dX);
    App_Printf("dR %d  dX %d\r\n", s_tMaixCAM_Data.dCoord_R, s_tMaixCAM_Data.dCoord_X);
    ...
    if (abs(s_tMaixCAM_Data.usCoord_X - usCircleCenter_X) <= 1 &&
        abs(s_tMaixCAM_Data.usCoord_Y - usCircleCenter_Y) <= 1)
    {
        // 坐标连续n次位于中心
        if (s_tMaixCAM_Data.ucStableCount < s_tMaixCAM_Data.ucCircleStableThreshold)
        {
            App_Printf("ucStableCount++\r\n");
            s_tMaixCAM_Data.ucStableCount++;
        }
        else   // 计数达到阈值，认为真正在中心
        {
    ...
    else
    {
        ...
        APP_MOTOR_ZDT_WaitAck(MOTOR_X, 200);
        APP_MOTOR_ZDT_WaitAck(MOTOR_R, 200);
        App_Printf("Dir[%d] X %d  R %d\r\n", DirMapping[Fir][s_MaixCAM_ID.ucColor], MOTOR_X_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], MOTOR_R_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]]);
...
void APP_MAIXCAM_Cal(uint8_t _ucElement, uint8_t _ucColor, uint8_t _ucIsCalFlag, uint8_t _ucMoveFlag)
{
    ...
    else if (s_tMaixCAM_Data.ucCalFlag == CircleCalFlag) 
    {
        ...
        else if (s_tMaixCAM_Data.ucMoveFlag == MoveXR)
        {
            ...
            APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 130, MOTOR_R_Pos[DirMapping[Fir][_ucColor]], 1, 0);
            APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 130, MOTOR_X_Pos[DirMapping[Fir][_ucColor]], 1, 0);
            //Y0：最高点
            APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, 500, 220, 8500, 1, 0);
            APP_MOTOR_ZDT_WaitAck(MOTOR_X, MOTOR_X_Timeout);
            APP_MOTOR_ZDT_WaitAck(MOTOR_R, 2000);
