...
//底盘速度环控制任务
void APP_CHASSIS_VelControl(void)
{
    while (1)
    {
        //底盘控制PID计算
...
void chassis_control_vector(float* vx_set, float* vy_set, chassis_move_t* chassis_move)
{
    ...
    float v2 = 50;
    float v_max = 0;
    int64_t dis_long = 799000;  //dis_set大于此值，以v1速度，反之v2速度
    float dis_dec_k = 0.6;     //减速距离的系数，行走到dis_dec_k*dis_set之后，开始减速
    ...
    if (chassis_move->move_mode == CHASSIS_LONG)
    {
        ...
        if (chassis_move->dir == CarDirection_Forward)     //长距离行走时速度是快的
        {
            v_max = 200;
        }
    }
...
//            v_max = 100;
            ...
            // 减速逻辑
            if (chassis_move->dis > dis_dec_k * chassis_move->dis_set) 
            {
                // 如果是第一次进入减速区间
                if (!chassis_move->is_decelerating) 
                {
                    chassis_move->initial_decel_v = chassis_move->chassis_cmd_slow_set_vx.out;
                    chassis_move->is_decelerating = 1;  // 标记已进入减速状态
                }
                
                float remain_dist = 0;
                float dec_ratio = 0;
                remain_dist = chassis_move->dis_set - chassis_move->dis;
                if (chassis_move->dis_set >= dis_long)     //长距离行走时减速是快的
                {
                     dec_ratio = remain_dist / (chassis_move->dis_set * (1 - dis_dec_k)) * 0.5;
                }
                else                                       //短距离行走时减速是慢的
                {
                     dec_ratio = remain_dist / (chassis_move->dis_set * (1 - dis_dec_k)) * 1.5;
                }
                dec_ratio = fp32_constrain(dec_ratio, 0.1f, 1.0f); // 保持最小10%速度
                 
                vx_set_channel = dec_ratio * chassis_move->initial_decel_v;
            }
            else
            {
                // 不在减速区间时重置标志
                chassis_move->is_decelerating = 0;
            }
        ...
        case CarDirection_Left:
            ...
            if (chassis_move->dis > dis_dec_k * chassis_move->dis_set) 
            {
                ...
                if (!chassis_move->is_decelerating) 
                {
                    chassis_move->initial_decel_v = chassis_move->chassis_cmd_slow_set_vy.out;
                    chassis_move->is_decelerating = 1;  // 标记已进入减速状态
                }
                ...
                vy_set_channel = dec_ratio * chassis_move->initial_decel_v;
            }
            else
            {
                // 不在减速区间时重置标志
...
void APP_CHASSIS_MovePID(chassis_move_mode_e _move_mode, chassis_block_mode_e _block_mode, uint8_t _dir, int64_t _clk)
{
    ...
    _clk *= k;
    chassis_move.chassis_mode = CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW;
    ...
    if (chassis_move.dir == CarDirection_Rotate)    //旋转
    {
        ...
        chassis_move.chassis_yaw_set = _clk / k + yaw_offset;    //夹爪指向的度数，逆时针为正
...
void APP_CHASSIS_Status(STATUS_E _ucStatus)
{
    switch (_ucStatus)
    {
        case STATUS_Start_2_Qr:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Left, 125);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 540);
            break;

        case STATUS_Qr_2_RawArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 820);
            break;

        case STATUS_RawArea_2_ProFirArea_1:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 370);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 270);       //夹爪朝向90度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 1650);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180);      //夹爪朝向180度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 500);
            break;

        case STATUS_ProFirArea_2_RawArea_1:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 370);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 90);       //夹爪朝向90度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 1650);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0);      //夹爪朝向180度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 500);
            break;
        ...
        case STATUS_ProFirArea_2_ProSecArea_3:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 270);       //夹爪朝向90度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 800);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180);       //夹爪朝向90度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 800);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 270);       //夹爪朝向90度
            break;

        case STATUS_ProSecArea_2_RawArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 860);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0);        //夹爪朝向0度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 430);
            break;

        case STATUS_ProSecArea_2_Start:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 850);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Right, 350);        //夹爪朝向0度
            break;
