...
//    .ucBlobCenterErrCal = 10,    //转盘区校准误差允许（范围小，以便精准校准位置）
    .ucBlobCenterErrCal = 1,    //转盘区校准误差允许（范围小，以便精准校准位置）
...
//    .ucBlobStableThreshold = 3,  //转盘区稳定次数阈值，调节该参数以使小车不会随物块移动
    ...
    .ucCircleCenterErrCal = 5,   //色环区校准误差允许
...
static void APP_MAIXCAM_Parse_BLOB_Cal(void)
{
    ...
    if (isInCenter)
    {
        ...
        s_tMaixCAM_Data.ucStableCount = 0;
        s_tMaixCAM_Data.usCoord_X = 0;
        s_tMaixCAM_Data.usCoord_Y = 0;
        s_usPresentX = 0;
        s_usPresentY = 0;
        s_usPreviousX = 0;
        s_usPreviousY = 0;
...
//        abs( s_tMaixCAM_Data.dCoord_Y ) <= s_tMaixCAM_Data.ucCircleCenterErrCal )
    ...
    {        
        ...
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD,  20, 0, MOTOR_X_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,      20, 0, MOTOR_R_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0); 
        vTaskDelay(400);
    }
...
static void SendToAPP_MotorWhale(void)
{
    ...
    ptCarXYR->WhalePosX = abs( s_tMaixCAM_Data.dCoord_X ) * 1200;    

    ptCarXYR->WhaleDirY = CARDIRECTION_Y(s_tMaixCAM_Data.dCoord_Y);
    ptCarXYR->WhalePosY = abs( s_tMaixCAM_Data.dCoord_Y ) * 1200;
...
{    
    ...
    {
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 0, MOTOR_R_Pos[MIDDLE], 1, 0); 
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 0, MOTOR_X_Pos[MIDDLE], 1, 0);
        vTaskDelay(2000);
    }
