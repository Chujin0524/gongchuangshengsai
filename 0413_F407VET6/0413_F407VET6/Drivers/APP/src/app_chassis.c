...
//        chassis_move_control->wz_set = 0; //忽略陀螺仪校准
    ...
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_TURN)    //废弃
    {
//        vx_set = 100.0f, vy_set = 10.0f;       //测试用
        float delat_angle = 0.0f;
        //设置底盘控制的角度
        delat_angle = theta_format(chassis_move.chassis_yaw_set - chassis_move.chassis_yaw);
        deadband_limit(delat_angle, delat_angle, CHASSIS_YAW_DEADLINE);
        ...
        // 减速逻辑
        float dec_ratio = delat_angle / 90;
        dec_ratio = fp32_constrain(dec_ratio, 0.7f, 1.0f); // 保持最小10%速度         
        dec_ratio = 1; // 保持最小10%速度         
        vx_set = -120;
        vy_set = 40;
...
//        vTaskDelay(pdMS_TO_TICKS(300));
    ...
    else if (chassis_move.dir == CarDirection_Turn)    //转弯
    {
        chassis_move.dis_set = chassis_move.dis_remaining = 0;
        chassis_move.chassis_yaw_set = _clk / k;    //夹爪指向的度数，逆时针为正
        chassis_move.chassis_mode = CHASSIS_VECTOR_TURN;
        float delat_angle = 0.0f;
        delat_angle = theta_format(chassis_move.chassis_yaw_set - chassis_move.chassis_yaw);     //计算角度偏差

        while (fabs(delat_angle) >= 5.0)    //当角度偏差大于0.1时阻塞在该程序
        {
            delat_angle = theta_format(chassis_move.chassis_yaw_set - chassis_move.chassis_yaw);
        }
        chassis_move.chassis_mode = CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW;
...
void APP_CHASSIS_Status(STATUS_E _ucStatus)
{
    switch (_ucStatus)
    {
        ...
        case STATUS_Qr_2_RawArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 860);
            break;
        ...
        case STATUS_ProSecArea_2_RawArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 850);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0);        //夹爪朝向0度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 400);
            break;

        case STATUS_ProSecArea_2_Start:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 850);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0);        //夹爪朝向0度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 1700);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Right, 300);
            break;
