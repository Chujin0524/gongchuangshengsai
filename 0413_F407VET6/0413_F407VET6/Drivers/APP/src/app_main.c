...
void APP_MainStatusRunning(void)
{
    ...
    static uint8_t ucaGrabSequence[6] = {2, 3, 1, 1, 3, 2};
...
//            s_ucStatus = STATUS_Idle;
        ...
        case STATUS_RawAreaGrab:
        {
            ...
            App_Printf("ENTER TickFlag == 2\r\n");
                ...
                vTaskDelay(pdMS_TO_TICKS(1300));   //待调
                APP_Handle_PickRawArea(ucaGrabSequence[2+3*ucCycle], BlobColorMappingDir[ucaGrabSequence[2+3*ucCycle]]);
                xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
            }
...
//            //细校准
//            APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 100, MOTOR_R_Pos[CIRCLE_R_MIDDLE], 1, 0);
//            APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 100, MOTOR_X_Pos[CIRCLE_X_MIDDLE], 1, 0);
//            //Y0：最高点
...
//            APP_MAIXCAM_Cal(CIRCLE, GREEN, CircleCalFlag, MoveCar);       //识别圆环进行校准，使小车移动
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
...
//            //识别红色圆环进行校准，使X R移动。结束后抓取一个
//            APP_MAIXCAM_Cal(CIRCLE, RED, CircleCalFlag, MoveXR);       
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            APP_Handle_Place_One(RED);
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            
//            //识别绿色圆环进行校准，使X R移动。结束后抓取一个
//            APP_MAIXCAM_Cal(CIRCLE, GREEN, CircleCalFlag, MoveXR);       
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            APP_Handle_Place_One(GREEN);
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            
//            //识别蓝色圆环进行校准，使X R移动。结束后抓取一个
//            APP_MAIXCAM_Cal(CIRCLE, BLUE, CircleCalFlag, MoveXR);       
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
...
//            else if (ucCycle == 1) s_ucStatus = STATUS_ProSecArea_2_Start;
        ...
        case STATUS_ProSecAreaCal:
            ...
            //状态动作 粗校准
            if (ucCycle == 0)      APP_MAIXCAM_Cal(CIRCLE, GREEN, CircleCalFlag, MoveCar);       //识别绿色圆环进行校准，使小车移动
            else if (ucCycle == 1) APP_MAIXCAM_Cal(BLOB, GREEN, CircleCalFlag, MoveCar);       //识别绿色圆环进行校准，使小车移动
            //后处理1，等待小车粗校准完成绿色色环
            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
...
//            s_ucStatus = STATUS_Idle;
        ...
        case STATUS_ProSecAreaPlace:
            ...
            //细校准
//            APP_MAIXCAM_Cal(CIRCLE, GREEN, CircleCalFlag, MoveXR);       //识别圆环进行校准，使小车移动
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
...
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            
//            APP_MAIXCAM_Cal(CIRCLE, RED, CircleCalFlag, MoveXR);       //识别圆环进行校准，使小车移动
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);

//            //根据校准之后的X、R，放置一个物块
//            APP_Handle_Place_One(RED, 0);
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
//            
//            APP_MAIXCAM_Cal(CIRCLE, BLUE, CircleCalFlag, MoveXR);       //识别圆环进行校准，使小车移动
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);

//            //根据校准之后的X、R，放置一个物块
//            APP_Handle_Place_One(BLUE, 0);
//            xTaskNotifyWait(0x00000000UL, 0xFFFFFFFFUL, &pulNotificationValue, portMAX_DELAY);
