...
PID_TypeDef Maxi_Cam_X;
PID_TypeDef Maxi_Cam_Y;
/*
*********************************************************************************************************
...
void APP_MAIXCAM_Init( void )
{
    ...
    DirMapping = APP_Handle_GetMapPoint();
		float pid[3]={0.5,0,0};
		PID_init(&Maxi_Cam_X, PID_POSITION, pid, 6000, 0.1);
		PID_init(&Maxi_Cam_Y, PID_POSITION, pid, 6000, 0.1);
}
...
static void APP_MAIXCAM_Parse_BLOB_Cal(void)
{
    ...
    {
				double err_x=s_usPresentX-s_tMaixCAM_Data.usBlobCenter_X;
				double err_y=s_usPresentY-s_tMaixCAM_Data.usBlobCenter_Y;
					//int speed_temp=(moto_chassis[0].speed_rpm+moto_chassis[1].speed_rpm+moto_chassis[2].speed_rpm+moto_chassis[3].speed_rpm)/4.0;
					Maxi_Cam_X.set=0;
					Maxi_Cam_X.Ki=0.2+0.6*(fabs(err_x)/90);
					PID_calc(&Maxi_Cam_X,err_x,0);
					PID_calc(&Maxi_Cam_Y,err_y,0);
					float w =Maxi_Cam_X.out;
        // 发送移动距离量到APP_MotorWhale的消息队列
        s_tMaixCAM_Data.dCoord_X = Maxi_Cam_X.out;
        s_tMaixCAM_Data.dCoord_Y = Maxi_Cam_Y.out;			
