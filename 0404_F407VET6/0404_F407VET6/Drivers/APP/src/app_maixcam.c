...
static uint32_t *MOTOR_Y_Pos = NULL;
...
MAIXCAM_DATA_T s_tMaixCAM_Data =
{
    ...
    .usBlobCenter_X = 174,       //转盘区中心X
    .usBlobCenter_Y = 57,       //转盘区中心Y
    //    .ucBlobCenterErrCal = 10,    //转盘区校准误差允许（范围小，以便精准校准位置）
    .ucBlobCenterErrCal = 5,    //转盘区校准误差允许（范围小，以便精准校准位置）
    ...
    .usCircleCenter_X = 138,     //中间色环区中心点X
    .usCircleCenter_Y = 115,     //中间色环区中心点Y

    .usRedCircleCenter_X = 169,     //红色环区中心点X
    .usRedCircleCenter_Y = 113,     //红色环区中心点Y
    .usGreenCircleCenter_X = 138,     //绿色环区中心点X
    .usGreenCircleCenter_Y = 115,     //绿色环区中心点Y
    .usBlueCircleCenter_X = 169,     //蓝色环区中心点X
    .usBlueCircleCenter_Y = 113,     //蓝色环区中心点Y
...
void APP_MAIXCAM_Init(void)
{
    ...
    MOTOR_Y_Pos = APP_Handle_GetPosPoint(MOTOR_Y);
    DirMapping = APP_Handle_GetMapPoint();

    float pid_1[3] = {0.8, 0, 0};
    ...
    switch (s_tMaixCAM_Data.ucStatus)
    {
        ...
        case MAIXCAM_STATUS_BLOB_Cal:
            if (APP_MAIXCAM_Parse_BLOB_Cal() == 1)
            {
                ...
                APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 250, 3750, 1, 0);
                APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 250, 8000, 1, 0);
                APP_MOTOR_ZDT_Move_P(MOTOR_W, MOTOR_W_Direction_CCR,     200, 100, 1400, 1, 0);
...
static uint8_t APP_MAIXCAM_Parse_BLOB_ArriveAndWaitCal(void)
{
    ...
    {
        if (abs(s_tMaixCAM_Data.usBlobCoord_X_Fir - s_tMaixCAM_Data.usCoord_X) < 20 & abs(s_tMaixCAM_Data.usBlobCoord_Y_Fir - s_tMaixCAM_Data.usCoord_Y) < 20)
        {
            return 1;
        }
    }
...
static uint8_t APP_MAIXCAM_Parse_BLOB_Cal(void)
{
    ...
    if ( !s_usPresentX )   //如果收到的X Y数据有一个为0，则说明相机没有识别到，让摄像头重新发送
    {
        s_tMaixCAM_Data.ucSendFlag = 0;
        return 0;
    }
...
static uint8_t APP_MAIXCAM_Parse_CIRCLE_CalXR(void)
{
    ...
    else
    {
        ...
        APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD,  50, 50, MOTOR_X_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
        APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,      50, 50, MOTOR_R_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
...
void APP_MAIXCAM_Cal(uint8_t _ucElement, uint8_t _ucColor, uint8_t _ucIsCalFlag, uint8_t _ucMoveFlag)
{
    ...
    if (s_tMaixCAM_Data.ucCalFlag == CircleCalFlag) 
    {
        ...
        else if (s_tMaixCAM_Data.ucMoveFlag == MoveXR)
        {
            ...
            APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     200, 100, MOTOR_R_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
            APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, 200, 100, MOTOR_X_Pos[DirMapping[Fir][s_MaixCAM_ID.ucColor]], 1, 0);
            //Y0：最高点
            APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, 500, 220, 7500, 1, 0);
            APP_MOTOR_ZDT_WaitAck(MOTOR_X);
            APP_MOTOR_ZDT_WaitAck(MOTOR_R);
