...
static void APP_BLUETOOTH_Parse(uint8_t* pucFrameData, uint8_t ucFrameLength)
{
    ...
    {
        APP_MOTOR_ZDT_Move_P(s_MotorZDT_ID, s_MotorZDT_DIR, 50, 100, s_MotorZDT_POS, 1, 0);

        ucFlag = Flag_MotorZDT;
    }
    else if (strncmp((char *)pucFrameData, "M_ZDTstart", 10) == 0)
    {
        APP_MOTOR_ZDT_Move_P(s_MotorZDT_ID, s_MotorZDT_DIR, 10, 10, s_MotorZDT_POS, 1, 0);
        APP_MOTOR_ZDT_WaitAck(s_MotorZDT_ID);
    ...
    else if (strncmp((char *)pucFrameData, "Handle_Test=", 12) == 0)
    {
        ...
        switch (test)
        {
            ...
            case 7:                  //放一个测试
                vTaskDelay(5000);
                APP_MOTOR_ZDT_Move_P(MOTOR_Y, 0, 800, 0, 10500, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y);  //降到最底
                APP_SERVO_Jaw('Z');        //夹紧
                APP_MOTOR_ZDT_Move_P(MOTOR_Y, 0, 800, 0, 300, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y);    //升到最高
                APP_SERVO_Wrist('R');
                vTaskDelay(400);     //手腕朝右
                APP_MOTOR_ZDT_Move_P(MOTOR_Y, 0, 800, 0, 3000, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y);   //降到转盘上分
                APP_SERVO_Jaw('S');       //松开
                APP_MOTOR_ZDT_Move_P(MOTOR_Y, 0, 800, 0, 300, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y);    //升到最高
                APP_SERVO_Wrist('F');
