...
void chassis_behaviour_control_set(float *vx_set, float *vy_set, float *angle_set, chassis_move_t *chassis_move)
{
    ...
    else if (chassis_behaviour_mode == CHASSIS_MOVE_AND_TURN) // 新增模式
    {
        chassis_move_and_turn_control(vx_set, vy_set, angle_set, chassis_move);
    }
}
...
/**
  ...
  * @param[in]      chassis_move: 底盘数据
  * @retval         无
  */
static void chassis_move_and_turn_control(float *vx_set, float *vy_set, float *angle_set, chassis_move_t *chassis_move)
{
    if (vx_set == NULL || vy_set == NULL || angle_set == NULL || chassis_move == NULL)
    {
        return;
    ...
    // 初始化状态
    if (chassis_move->move_and_turn_init_flag)
    {
        chassis_move->start_yaw = chassis_move->chassis_yaw;
        chassis_move->start_distance = chassis_move->dis;  // 记录起始距离
        chassis_move->move_and_turn_init_flag = 0;
        
        // 计算单位距离的角度变化量
        if (chassis_move->dis_set > 0)
        {
            chassis_move->angle_per_distance = (chassis_move->chassis_yaw_set - chassis_move->start_yaw) / 
                                             (float)chassis_move->dis_set;
        }
        else
        {
            chassis_move->angle_per_distance = 0.0f;
        }
    }
    ...
    // 计算当前期望的角度 (基于移动距离线性变化)
    float current_target_yaw = chassis_move->start_yaw + 
                              chassis_move->angle_per_distance * moved_distance;
    
//    // 计算角度偏差
//    float angle_error = theta_format(current_target_yaw - chassis_move->chassis_yaw);
//    
//    // 设置角速度
//    chassis_move->wz_set = PID_calc(&chassis_move->chassis_angle_pid, 0.0f, angle_error);
    
    // 计算移动速度
    chassis_control_vector(vx_set, vy_set, chassis_move);
    
    // 设置输出角度
    *angle_set = current_target_yaw;
}
