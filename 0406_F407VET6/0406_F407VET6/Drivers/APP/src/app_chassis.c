...
static void chassis_init(chassis_move_t* chassis_move_init)
{
    ...
    chassis_move_init->vy_min_speed = -NORMAL_MAX_CHASSIS_SPEED_Y;
		...
		// 初始化运动控制相关参数
    chassis_move_init->move_init_flag = 0;
    chassis_move_init->acceleration_distance = 0;
    chassis_move_init->deceleration_distance = 0;
    chassis_move_init->cruise_distance = 0;
    chassis_move_init->current_max_speed = 0;
    chassis_move_init->acceleration = 0;
    chassis_move_init->deceleration = 0;
    chassis_move_init->start_distance = 0;
...
void chassis_control_vector(float* vx_set, float* vy_set, chassis_move_t* chassis_move)
{
    ...
    // 初始化运动参数
    if (chassis_move->move_init_flag)
    {
        float max_speed = (chassis_move->move_mode == CHASSIS_LONG) ? 180.0f : 50.0f;
        float total_distance = (float)chassis_move->dis_set;
        
        // 计算最小加速/减速距离
        float min_accel_distance = (max_speed * max_speed) / (2.0f * CHASSIS_ACCELERATION);
        float min_decel_distance = (max_speed * max_speed) / (2.0f * CHASSIS_DECELERATION);
        
        // 判断是否能够达到最大速度
        if ((min_accel_distance + min_decel_distance) > total_distance)
        {
            // 三角速度曲线
            float ratio = sqrtf(total_distance / (min_accel_distance + min_decel_distance));
            chassis_move->current_max_speed = max_speed * ratio;
            chassis_move->acceleration_distance = total_distance * (min_accel_distance / (min_accel_distance + min_decel_distance));
            chassis_move->deceleration_distance = total_distance - chassis_move->acceleration_distance;
            chassis_move->cruise_distance = 0.0f;
        }
        else
        {
            // 梯形速度曲线
            chassis_move->current_max_speed = max_speed;
            chassis_move->acceleration_distance = min_accel_distance;
            chassis_move->deceleration_distance = min_decel_distance;
            chassis_move->cruise_distance = total_distance - chassis_move->acceleration_distance - chassis_move->deceleration_distance;
        }
        
        chassis_move->acceleration = CHASSIS_ACCELERATION;
        chassis_move->deceleration = CHASSIS_DECELERATION;
        chassis_move->move_init_flag = 0;
        chassis_move->start_distance = chassis_move->dis; // 记录起始距离
    }

    float moved_distance = (float)(chassis_move->dis - chassis_move->start_distance);
    float v_target = 0.0f;

    // 计算目标速度
    if (moved_distance < 0)
    {
        v_target = 0.0f; // 还未开始移动
    }
    else if (moved_distance < chassis_move->acceleration_distance)
    {
        // 加速阶段
        v_target = sqrtf(2.0f * chassis_move->acceleration * moved_distance);
    }
    else if (moved_distance < (chassis_move->acceleration_distance + chassis_move->cruise_distance))
    {
        // 匀速阶段
        v_target = chassis_move->current_max_speed;
    }
    else if (moved_distance < (chassis_move->dis_set - chassis_move->start_distance))
    {
        // 减速阶段
        float decel_start = chassis_move->acceleration_distance + chassis_move->cruise_distance;
        float decel_dist = moved_distance - decel_start;
        v_target = chassis_move->current_max_speed - sqrtf(2.0f * chassis_move->deceleration * decel_dist);
    }
    else
    {
        // 已经到达终点
    ...
    // 确保速度在合理范围内
    v_target = fp32_constrain(v_target, 0.0f, chassis_move->current_max_speed);
...
void APP_CHASSIS_Status(STATUS_E _ucStatus)
{
    switch (_ucStatus)
    {
        case STATUS_Start_2_Qr:
            APP_CHASSIS_MovePID(CHASSIS_CAL, CHASSIS_CALL_BLOCKING, CarDirection_Left, 125);
        ...
        case STATUS_RawArea_2_ProFirArea:
            APP_CHASSIS_MovePID(CHASSIS_CAL, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 380);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 90);       //夹爪朝向90度
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Forward, 1750);
        ...
        case STATUS_ProFirArea_2_ProSecArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 800);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Left, 900);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 90);       //夹爪朝向90度
            break;

        case STATUS_ProSecArea_2_RawArea:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 960);
            APP_CHASSIS_MovePID(CHASSIS_CAL, CHASSIS_CALL_BLOCKING, CarDirection_Left, 450);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 0);        //夹爪朝向0度
            break;

        case STATUS_ProSecArea_2_Start:
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Backward, 900);
            APP_CHASSIS_MovePID(CHASSIS_LONG, CHASSIS_CALL_BLOCKING, CarDirection_Rotate, 180);      //夹爪朝向180度
