...
static uint16_t MOTOR_W_Vel = 250;
...
//Y位置
static uint8_t MOTOR_Y_Acc[2] = {247, 0};         //0下降 1上升
static uint16_t MOTOR_Y_Vel[2] = {800, 900};       //0下降 1上升
...
void APP_Handle_Place_All(uint8_t _ucCycle, uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    //Y1：最高点-300
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[1], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
void APP_Handle_Place_One(uint8_t _color)
{
    ...
    //Y4：夹着物块放到色环上的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[4]-1500, 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
    ...
    //Y6：物块在色环上，夹爪能越过的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[6], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
void APP_Handle_Pick_All(uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    for (uint8_t i = 0; i < 3; i++)
    {
        ...
        //Y4：夹着物块放到色环上的位置
        APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[1], MOTOR_Y_Pos[4], 1, 0);
        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
        ...
        //Y3：夹着物块放到转盘上的位置
        APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[1], MOTOR_Y_Pos[3], 1, 0);
        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
//        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, 200);
