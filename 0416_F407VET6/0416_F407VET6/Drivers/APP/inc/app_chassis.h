...
#define CHASSIS_ACCEL_X_NUM 0.666666f
#define CHASSIS_ACCEL_Y_NUM 0.666666f

#define CHASSIS_ACCEL_X_NUM_CAL 0.888888f
#define CHASSIS_ACCEL_Y_NUM_CAL 0.888888f
...
#define M2006_MOTOR_SPEED_PID_KP 470.0f
#define M2006_MOTOR_SPEED_PID_KI 70.0f
...
//底盘旋转跟随PID
#define CHASSIS_FOLLOW_CHASSIS_PID_KP 3.0f
...
#define CarDirection_Turn      (0x03)       //转弯      3
...
#define CARDIRECTION_BLOB_Y(Y)     (Y>0) ? CarDirection_Left    :  CarDirection_Right
#define CARDIRECTION_BLOB_X(X)     (X>0) ? CarDirection_Forward :  CarDirection_Backward

#define CARDIRECTION_CIRCLE_Y(Y)     (Y>0) ? CarDirection_Left     :  CarDirection_Right
#define CARDIRECTION_CIRCLE_X(X)     (X>0) ? CarDirection_Backward :  CarDirection_Forward
...
#define CHASSIS_ACCELERATION  (200.0f)  // 加速度 (单位: mm/s²)
#define CHASSIS_DECELERATION  (200.0f)  // 减速度 (单位: mm/s²)

typedef enum
{
    CHASSIS_CALL_BLOCKING=0,       //阻塞调用
...
typedef struct
{
    ...
    uint8_t move_and_turn_init_flag;    // 边走边转初始化标志
    float start_yaw;                    // 起始角度
    float start_distance;               // 起始距离
    float angle_per_distance;           // 单位距离角度变化量
		
		float acceleration_distance;    // 加速段距离
    float deceleration_distance;    // 减速段距离
    float cruise_distance;          // 匀速段距离
    float current_max_speed;        // 当前最大速度
    float acceleration;             // 加速度
    float deceleration;             // 减速度
		
		uint8_t move_init_flag;         // 运动初始化标志
		
    chassis_move_mode_e move_mode;  //移动模式
    
    uint8_t is_decelerating;  // 减速状态标志
    float initial_decel_v;   // 初始减速速度保存
} chassis_move_t;
