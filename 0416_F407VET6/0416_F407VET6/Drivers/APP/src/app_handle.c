...
腕处位置 :
*/
uint8_t MOTOR_W_Acc[2] = {140, 200};
uint16_t MOTOR_W_Vel = 300;
uint32_t MOTOR_W_Pos[10] =
{
    0,        //0 : 初始
...
转盘位置 :
*/
uint8_t MOTOR_R_Acc = 220;
uint16_t MOTOR_R_Vel = 300;
uint32_t MOTOR_R_Pos[7] =
{
    0,        //0 : 初始
...
X位置 :
*/
uint8_t MOTOR_X_Acc = 245; //0-缩  1-伸
uint16_t MOTOR_X_Vel = 450;
uint32_t MOTOR_X_Pos[8] =
{
    0,        //0 : 初始
...
//Y位置
uint8_t MOTOR_Y_Acc[2] = {250, 255};         //0下降 1上升
uint16_t MOTOR_Y_Vel[2] = {800, 1200};       //0下降 1上升
uint32_t MOTOR_Y_Pos[9] =
{
    0,        //0 : 最高点
    100,        //1 : 最高点-300
    100,        //2 : 夹着物块能越过转盘的位置
    2600,     //3 : 夹着物块放到转盘上的位置
    12000,    //4 : 夹着物块放到色环上的位置
    6300,     //5 : 码垛物块的位置
...
{
    uint32_t Delay_Wrist = 700;
    uint8_t MOTOR_X_Acc = 237;
...
//            vTaskDelay(pdMS_TO_TICKS(Delay_Wrist));
    ...
    APP_MOTOR_ZDT_WaitAck(MOTOR_R, Delay_Wrist);

    __NOP();
    //Y8：从转盘区夹起物块的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[8], 1, 0);
    ...
    //Y2：夹着物块能越过转盘的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[2], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
    ...
    //腕处转到物料盘上方
    APP_MOTOR_ZDT_Move_P(MOTOR_W, MOTOR_W_Direction_CCR,     MOTOR_W_Vel, MOTOR_W_Acc[1]-10, MOTOR_W_Pos[RIGHT_W], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_W, Delay_Wrist);
//    APP_MOTOR_ZDT_WaitAck(MOTOR_W);

    //Y3：夹着物块放到物料盘上的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[3], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

    APP_SERVO_Jaw('S');     //夹爪舵机松开物块

    //Y0：最高点
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[0], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
void APP_Handle_Place_All(uint8_t _ucCycle, uint8_t _firstIndex, uint8_t _secondIndex, uint8_t _thirdIndex)
{
    ...
    //Y1：最高点-300
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[1], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

    for (uint8_t i = 0; i < 3; i++)
    {
        if (ucSeq[i] == LEFT && _ucCycle == 1) 
        {
            APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     MOTOR_R_Vel, MOTOR_R_Acc, MOTOR_R_Pos[ucSeq[i]]+1500, 1, 0);
        
            //X导轨伸出到指定色环
            APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, MOTOR_X_Vel, MOTOR_X_Acc, MOTOR_X_Pos[ucSeq[i]], 1, 0);

            //腕处舵机朝右
            APP_MOTOR_ZDT_Move_P(MOTOR_W, MOTOR_W_Direction_CCR,     MOTOR_W_Vel, MOTOR_W_Acc[0], MOTOR_W_Pos[RIGHT_W], 1, 0);
            APP_MOTOR_ZDT_WaitAck(MOTOR_W, Delay_Wrist);
            ...
            //Y2：夹着物块能越过转盘的位置
            APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[2], 1, 0);
            APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
        ...
        if (_ucCycle == 0)
        {
            ...
            if (ucSeq[i] == RIGHT) 
            {
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]-400, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

//                //Y4：夹着物块放到色环上的位置
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, 100, 50, MOTOR_Y_Pos[4]+100, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]+100, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
            }
            else if (ucSeq[i] == LEFT)
            {
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]-850, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

//                //Y4：夹着物块放到色环上的位置
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, 100, 50, MOTOR_Y_Pos[4]-350, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]-350, 1, 0);
                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
            }
            else
            {
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]-650, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);

//                //Y4：夹着物块放到色环上的位置
//                APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, 100, 50, MOTOR_Y_Pos[4]-50, 1, 0);
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
        ...
        {
            //Y5：码垛物块的位置
            APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[5], 1, 0);
            APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
        }

        //夹爪舵机松开物块，内有延迟
        if (i==0)      APP_SERVO_Plate(_secondIndex);        //转盘舵机转到对应物块的放置角度
        else if (i==1) APP_SERVO_Plate(_thirdIndex);        //转盘舵机转到对应物块的放置角度
        APP_SERVO_Jaw('S');

        //Y0：最高点
        APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-5, MOTOR_Y_Pos[0], 1, 0);
        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
    }
...
void APP_Handle_Place_One(uint8_t _color, uint8_t _next_color)
{
    ...
    //Y3：夹着物块放到转盘上的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-5, MOTOR_Y_Pos[3], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
//    //Y4：夹着物块放到色环上的位置
//    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-5, MOTOR_Y_Pos[4]-1500, 1, 0);
//    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
        ...
        APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0]-10, MOTOR_Y_Pos[4]+50, 1, 0);
        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
    }
    else if (ucSeq == LEFT)
    {
...
//                APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
    ...
    //夹爪舵机松开物块，内有延迟
    APP_SERVO_Plate(_next_color);        //转盘舵机转到对应物块的放置角度
    APP_SERVO_Jaw('S');

    //Y6：物块在色环上，夹爪能越过的位置
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[6], 1, 0);
    APP_MOTOR_ZDT_WaitAck(MOTOR_Y, MOTOR_Y_Timeout);
...
//        APP_SERVO_Jaw('S');     //夹爪舵机松开物块
        ...
        //Y4：夹着物块放到色环上的位置
        APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN, MOTOR_Y_Vel[0], MOTOR_Y_Acc[0], MOTOR_Y_Pos[4], 1, 0);
        APP_MOTOR_ZDT_WaitAck(MOTOR_Y, 700);
...
void APP_Handle_Run(void)
{
    uint8_t MOTOR_X_Acc = 237;
		...
		APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN,    MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[0], 1, 0);
    APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     MOTOR_R_Vel, MOTOR_R_Acc, 3700, 1, 0);
    APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_FORWARD, MOTOR_X_Vel, MOTOR_X_Acc, 5000, 1, 0);
...
void APP_Handle_Start(void)
{
    __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_PLATE, 1040);
    HAL_Delay(500);
    APP_MOTOR_ZDT_Move_P(MOTOR_Y, MOTOR_Y_Direction_DOWN,    MOTOR_Y_Vel[1], MOTOR_Y_Acc[1], MOTOR_Y_Pos[0], 1, 0);
    APP_MOTOR_ZDT_Move_P(MOTOR_R, MOTOR_R_Direction_CCR,     MOTOR_R_Vel, MOTOR_R_Acc, 3700, 1, 0);
    APP_MOTOR_ZDT_Move_P(MOTOR_X, MOTOR_X_Direction_BACKWARD,MOTOR_X_Vel, MOTOR_X_Acc, 500, 1, 0);
