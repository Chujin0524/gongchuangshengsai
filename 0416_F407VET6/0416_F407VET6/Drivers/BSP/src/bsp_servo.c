...
static uint16_t s_AnglePlate_R = 290;   //转盘角度  红色物块存放位置
static uint16_t s_AnglePlate_G = 1190;   //转盘角度  绿色物块存放位置
static uint16_t s_AnglePlate_B = 730;  //转盘角度  蓝色物块存放位置
static uint16_t s_AngleJaw_Z   = 670;     //夹爪角度  抓紧物块
//static uint16_t s_AngleJaw_S   = 1050;     //夹爪角度  松开物块
static uint16_t s_AngleJaw_S   = 920;     //夹爪角度  松开物块
static uint16_t s_AngleJaw_S2   = 920;     //夹爪角度  松开物块
static uint16_t s_AngleWrist_F = 1180;      //腕处角度  朝前
static uint16_t s_AngleWrist_R = 610;       //腕处角度  朝右

static uint16_t s_AngleJaw   = 1150;  //夹爪初始角度
...
void bsp_InitServo(void)
{
    ...
    __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_JAW, 820);
...
void SERVO_Jaw(uint8_t _ucIndex)
{
    ...
    case 's':   //松
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_JAW, s_AngleJaw_S2);
        break;
